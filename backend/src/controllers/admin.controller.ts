    ...
    try {
        ...
        const transaction = await prisma.$transaction(async (tx) => {
            ...
            if (action === 'SET_PLAN') {
                ...
                // 30 days default for manual set
                ...
                let features = '[]';
                if (['PRO', 'ENTERPRISE', 'FREE_1Y'].includes(plan)) {
                ...
                // If extending for one year, we TREAT it as the welcome package (Full Access)
                const isOneYear = Number(extendDays) >= 365;

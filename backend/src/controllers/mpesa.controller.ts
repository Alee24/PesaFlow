...
export const retryPayment = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        // Re-initiate B2C payment
        console.log(`[RETRY] Re-initiating B2C for transaction ${transactionId}`);
        const response = await initiateB2CPayment(
            transaction.metadata ? JSON.parse(transaction.metadata).phoneNumber || transaction.reference : transaction.reference!,
            Number(transaction.amount),
            `Retry-${transaction.reference}`,
            req.user.userId,
            `Retry of ${transaction.reference}`
        );

        // Optional: Mark old transaction as ARCHIVED or link them
        await prisma.transaction.update({
            where: { id: transaction.id },
            data: { 
                status: 'FAILED', // Ensure it's marked as failed if it was pending
                metadata: JSON.stringify({ ...JSON.parse(transaction.metadata || '{}'), retriedAs: response.ConversationID })
            }
        });

        res.json({ success: true, message: 'Retry initiated successfully', data: response });

...
# Mpesa Connect / Mpesa Connect - Installer & Setup Script
# Usage: ./install.sh
...
pm2 delete Mpesa Connect-backend 2>/dev/null || true
pm2 start dist/app.js --name Mpesa Connect-backend || pm2 start dist/server.js --name Mpesa Connect-backend
...
pm2 delete Mpesa Connect-frontend 2>/dev/null || true
pm2 start npm --name Mpesa Connect-frontend -- start -- -p 3000
...
echo "👉 Backend running (Mpesa Connect-backend)"
echo "👉 Frontend running (Mpesa Connect-frontend)"
